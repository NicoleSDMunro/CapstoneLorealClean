...
def calculate_decision_window(trigger_row,revisao_mes,hb_dias,lt_sem,po_emitido="Não",po_status="Aberto"):
    # Sprint 5 / Bloco 2: apenas viabilidade de ação. Assunção simples e defensável: 1 mês ≈ 30 dias.
    ...
    if dentro_hb or dentro_lt or po_confirmado:
        estado,emoji,classe="EXCESSO COMPROMETIDO","🔴","red";texto="Comprometido: compras/produção já estão travadas por HB, LT ou PO confirmado."
    elif entrando_lt:
        estado,emoji,classe="ÚLTIMA JANELA","🟡","yellow";texto="Ainda dá para agir, mas está entrando no LT. A decisão é possível com risco."
    else:
        estado,emoji,classe="AÇÃO POSSÍVEL","🟢","green";texto="Ainda dá para agir: o gatilho está fora do HB e fora do LT."
    hb_txt=f"Dentro ({dias_ate}d ≤ {hb_dias:.0f}d)" if dentro_hb else f"Fora ({dias_ate}d > {hb_dias:.0f}d)"
    if dentro_lt:lt_txt=f"Dentro ({dias_ate}d ≤ {lt_dias:.0f}d)"
    elif entrando_lt:lt_txt=f"Última janela ({dias_ate}d; LT {lt_dias:.0f}d + 30d)"
    else:lt_txt=f"Fora ({dias_ate}d > {lt_dias+30:.0f}d)"
...
def render_decision_table(df):
    visible=["Mês do gatilho","Status decisão","HB","LT","PO","Interpretação"]
    rows=""
    for _,r in df.iterrows():
        rows+=f"<tr class='{esc(r['Classe'])}'>"+"".join([f"<td class='{'status' if c=='Status decisão' else 'muted'}'>{esc(r[c])}</td>" for c in visible])+"</tr>"
    head="".join([f"<th>{c}</th>" for c in visible])
...
if not excess_triggers.empty:
    ...
    decision_rows=[calculate_decision_window(row,rev,hb_dias,lt_sem,po_emitido,po_status) for _,row in excess_triggers.iterrows()]
    decision_df=pd.DataFrame(decision_rows)
    counts=decision_df["Status decisão"].str.replace("🟢 ","",regex=False).str.replace("🟡 ","",regex=False).str.replace("🔴 ","",regex=False).value_counts().to_dict()
    ...
    render_decision_table(decision_df)
