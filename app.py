...
def calculate_decision_window(trigger_row,revisao_mes,hb_dias,lt_sem,po_emitido="Não",po_status="Aberto"):
    ...
    if dentro_hb or dentro_lt or po_confirmado:
        estado,emoji="EXCESSO COMPROMETIDO","🔴";texto="Comprometido: compras/produção já estão travadas por HB, LT ou PO confirmado."
    elif entrando_lt:
        estado,emoji="ÚLTIMA JANELA","🟡";texto="Ainda dá para agir, mas está entrando no LT. A decisão é possível com risco."
    else:
        estado,emoji="AÇÃO POSSÍVEL","🟢";texto="Ainda dá para agir: o gatilho está fora do HB e fora do LT."
    ...
    return {"Mês do gatilho":trigger_row["Mês"],"Status decisão":f"{emoji} {estado}","HB":hb_txt,"LT":lt_txt,"PO":po_txt,"Interpretação":texto}
...
def style_decision_table(df):
    def row_style(row):
        status=str(row["Status decisão"])
        if "🔴" in status:return ["background-color: rgba(239,68,68,.16); color:#f5f5f5"]*len(row)
        if "🟡" in status:return ["background-color: rgba(250,204,21,.13); color:#f5f5f5"]*len(row)
        return ["background-color: rgba(34,197,94,.12); color:#f5f5f5"]*len(row)
...
if not excess_triggers.empty:
    ...
    decision_df=pd.DataFrame(decision_rows)[["Mês do gatilho","Status decisão","HB","LT","PO","Interpretação"]]
    ...
    st.dataframe(style_decision_table(decision_df),use_container_width=True,hide_index=True,height=min(360,80+len(decision_df)*46))
