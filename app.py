...
html,body,[class*="css"]{font-family:'Inter',sans-serif}.stApp{background:#000;color:#f5f5f5}.block-container{padding-top:.25rem;max-width:1188px}[data-testid="stHeader"]{background:transparent}#MainMenu,footer{visibility:hidden}.hero-title{font-size:34px;line-height:.92;font-weight:900;letter-spacing:-1.5px;text-transform:uppercase;margin-bottom:10px;color:#fff;white-space:nowrap;overflow:hidden}.hero-subtitle{color:#9ca3af;font-size:14px;margin-bottom:28px}.empty-panel{background:#101010;border:1px dashed #3a3a3a;border-radius:10px;padding:34px 28px;margin-top:22px}.empty-title{font-size:24px;font-weight:850;margin-bottom:8px}.empty-text{color:#a5a5a5;font-size:15px;line-height:1.6}.stSelectbox label{color:#60646f!important;text-transform:none;font-size:14px}.stSelectbox [data-baseweb="select"]>div{background:#151515!important;border:1px solid #333!important;border-radius:6px!important;color:#e5e7eb!important}.stSelectbox [data-baseweb="select"] span{color:#e5e7eb!important;font-weight:550}.struct-box{min-height:96px;padding:16px 16px 14px;border:1px solid #2a2a2a;box-sizing:border-box;background:#101010}.label{color:#666;font-size:13px;text-transform:uppercase;letter-spacing:1.5px;font-weight:500}.value{color:#fff;font-size:18px;font-weight:800;margin-top:7px;white-space:nowrap}.sub{color:#9ca3af;font-size:13px;margin-top:7px;white-space:nowrap}.orange{color:#e8a24a}.red{color:#f05a5f}.green{color:#1ca37e}.tab-single{margin:34px 0 22px}.tab-single span{display:inline-block;padding:0 26px 14px 26px;text-transform:uppercase;letter-spacing:2px;font-size:15px;font-weight:800;color:#fff;border-bottom:4px solid #fff}.review-title{font-size:20px;font-weight:900;letter-spacing:.3px;margin:10px 0 18px}.month-row div[data-testid="stButton"] button,[data-testid="stButton"] button[kind="secondary"]{height:38px!important;border-radius:8px!important;border:1px solid #333!important;background:#171717!important;color:#d1d5db!important;font-weight:650!important;box-shadow:none!important}.month-row div[data-testid="stButton"] button p,[data-testid="stButton"] button[kind="secondary"] p{color:#d1d5db!important}.month-row div[data-testid="stButton"] button:hover,[data-testid="stButton"] button[kind="secondary"]:hover{background:#222!important;border-color:#454545!important;color:#fff!important}.month-row div[data-testid="stButton"] button[kind="primary"]{background:var(--accent)!important;color:#fff!important;border-color:var(--accent)!important;font-weight:800!important}.month-row div[data-testid="stButton"] button[kind="primary"] p{color:#fff!important;font-weight:800!important}.section-title{color:#9a9a9a;text-transform:uppercase;letter-spacing:2px;font-size:15px;font-weight:500;margin:24px 0 10px}.chart-card{background:#101010;border:1px solid #2a2a2a;border-radius:8px;padding:8px 12px 2px}.alert-box,.warn-box,.success-box{border-radius:8px;padding:16px 20px;color:#fff;font-size:16px;line-height:1.5;margin-bottom:10px}.alert-box{background:linear-gradient(90deg,rgba(239,68,68,.22),rgba(239,68,68,.05));border-left:6px solid #ef4444}.warn-box{background:linear-gradient(90deg,rgba(250,204,21,.18),rgba(250,204,21,.04));border-left:6px solid #facc15}.success-box{background:linear-gradient(90deg,rgba(22,163,123,.20),rgba(22,163,123,.04));border-left:6px solid #16a37b}.trigger-pills{display:flex;flex-wrap:wrap;gap:14px;margin:12px 0 16px}.trigger-pill-red{border:1px solid #ef4444;color:#ef4444;border-radius:999px;padding:8px 14px}.trigger-pill-orange{border:1px solid #f97316;color:#f97316;border-radius:999px;padding:8px 14px}.trigger-pill-yellow{border:1px solid #d49100;color:#d49100;border-radius:999px;padding:8px 14px}.decision-table-wrap{background:#101010;border:1px solid #2a2a2a;border-radius:10px;padding:14px;margin:10px 0 16px}.decision-note{color:#9ca3af;font-size:13px;line-height:1.5;margin:0 0 10px}.decision-summary{display:flex;gap:10px;flex-wrap:wrap;margin:8px 0 12px}.decision-chip{border:1px solid #303030;border-radius:999px;padding:7px 12px;color:#d1d5db;background:#0b0b0b;font-size:13px}.decision-table{width:100%;border-collapse:separate;border-spacing:0;overflow:hidden;border-radius:8px;border:1px solid #2a2a2a;background:#111}.decision-table th{background:#1a1a1a;color:#9ca3af;text-align:left;padding:12px 10px;font-size:13px;font-weight:700;border-bottom:1px solid #2a2a2a}.decision-table td{background:#111;color:#d1d5db;padding:12px 10px;font-size:13px;border-bottom:1px solid #242424;vertical-align:top}.decision-table tr:last-child td{border-bottom:0}.decision-table tr.green td{background:rgba(34,197,94,.10)}.decision-table tr.yellow td{background:rgba(250,204,21,.11)}.decision-table tr.red td{background:rgba(239,68,68,.13)}.decision-table td:first-child{font-weight:700;color:#f5f5f5}.decision-table .status{font-weight:800;color:#f5f5f5;white-space:nowrap}
</style>
...
def parse_sheet(xls, name):
    try:
        df = pd.read_excel(xls, sheet_name=name); df.columns = [str(c).strip() for c in df.columns]
        return df
    except Exception:
        return pd.DataFrame()
...
def calculate_decision_window(trigger_row, revisao_mes, hb_dias, lt_sem, po_emitido="Não", po_status="Aberto"):
    ...
    hb_dias = float(pd.to_numeric(hb_dias, errors="coerce") if pd.notna(hb_dias) else 60)
    lt_dias = float(pd.to_numeric(lt_sem, errors="coerce") if pd.notna(lt_sem) else 16) * 7
    dias_ate = max(0, int((trigger_period - rev_period).n * 30)) if rev_period and trigger_period else 0
    dentro_hb = dias_ate <= hb_dias; dentro_lt = dias_ate <= lt_dias; entrando_lt = lt_dias < dias_ate <= lt_dias + 30
    po_confirmado = po_emitido == "Sim" and po_status in ["Em produção","Em trânsito"]
    if dentro_hb or dentro_lt or po_confirmado:
        estado,emoji,classe,texto = "EXCESSO COMPROMETIDO","🔴","red","Comprometido: compras/produção já estão travadas por HB, LT ou PO confirmado."
    elif entrando_lt:
        estado,emoji,classe,texto = "ÚLTIMA JANELA","🟡","yellow","Ainda dá para agir, mas está entrando no LT. A decisão é possível com risco."
    else:
        estado,emoji,classe,texto = "AÇÃO POSSÍVEL","🟢","green","Ainda dá para agir: o gatilho está fora do HB e fora do LT."
...
def struct_box(label, value, sub, cls):
    st.markdown(f'<div class="struct-box"><div class="label">{label}</div><div class="value {cls}">{value}</div><div class="sub">{sub}</div></div>', unsafe_allow_html=True)
...
def render_decision_table(df):
    ...
    for _, r in df.iterrows():
        rows += f"<tr class='{html.escape(r['Classe'])}'>" + "".join([f"<td class='{'status' if c=='Status decisão' else ''}'>{html.escape(str(r[c]))}</td>" for c in cols]) + "</tr>"
...
if arquivo is None:
    st.markdown('<div class="empty-panel"><div class="empty-title">Nenhuma base carregada</div><div class="empty-text">Envie um arquivo Excel no menu lateral para liberar o dashboard.</div></div>', unsafe_allow_html=True)
    st.stop()

try:
    xls = pd.ExcelFile(arquivo)
    base_df = parse_sheet(xls, "Base"); bom_df = parse_sheet(xls, "BOM")
    partes, erros = [], []
...
st.markdown(f"<style>:root{{--accent:{cor_revisao(rev)};}}</style>", unsafe_allow_html=True)
...
hb_dias = float(pd.to_numeric(info["HBdias"], errors="coerce") if pd.notna(info["HBdias"]) else 60)
...
items = [("ORIGEM",info["Origem"],"","orange"),("LT MAX",f"{lt_sem:.0f} sem",f"aprox {lt_meses:.1f}m - {lt_dias}d",lt_cls),("COB. TARGET",f"{ct_dias:.0f}d",f"aprox {ct_dias/30:.1f}m - {ct_dias/7:.1f}sem",ct_cls),("DEMANDA",info["Demanda"],"",dem_cls),("EST. FINAL",fmt(est_final),"ultima rev - jun/26",est_cls)]
for i,(lab,val,sub,cls) in enumerate(items):
    with scols[i]: struct_box(lab,val,sub,cls)
...
with cols[-2]:
    if st.button("<", use_container_width=True):
        st.session_state.revisao = revisoes[max(0, revisoes.index(rev)-1)]; st.rerun()
with cols[-1]:
    if st.button(">", use_container_width=True):
        st.session_state.revisao = revisoes[min(len(revisoes)-1, revisoes.index(rev)+1)]; st.rerun()
...
if not excess_triggers.empty:
    ...
    with st.expander("Inputs opcionais de PO", expanded=False):
        po_emitido = st.selectbox("PO já emitido?", ["Não","Sim"], index=0)
        po_status = st.selectbox("Status do PO", ["Aberto","Em produção","Em trânsito"], index=0)
    decision_df = pd.DataFrame([calculate_decision_window(row, rev, hb_dias, lt_sem, po_emitido, po_status) for _, row in excess_triggers.iterrows()])
...
if prev_idx >= 0 and prev is not None:
    ...
    fig_var.add_hline(y=0,line_width=1,line_color="#6b7280"); fig_var.add_hline(y=10,line_width=1,line_dash="dash",line_color="#facc15"); fig_var.add_hline(y=-10,line_width=1,line_dash="dash",line_color="#facc15")
